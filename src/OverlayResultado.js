    ...
    <Pressable style={[styles.fondo, { backgroundColor: color.fondo }]} onPress={onContinuar}>
      ...
      {tipo === 'ok' && datos ? (
        <ScrollView style={styles.detalle} contentContainerStyle={styles.detalleContenido} showsVerticalScrollIndicator={false}>
          ...
          {pagoIncompleto ? (
            <View style={styles.pagoAviso}>
              <Text style={styles.pagoAvisoTexto}>⚠ PAGO INCOMPLETO</Text>
              <Text style={styles.pagoAvisoDetalle}>Recordá solicitar el pago completo.</Text>
            </View>
          ) : null}
            ...
            <>
              ...
              {datos.talleres.map((t, i) => (
                <Text key={i} style={styles.taller}>
                  {pagoIncompleto && t.pago !== 'pago_completo' ? '⚠ ' : ''}• {t.taller}
...
const styles = StyleSheet.create({
  ...
  detalleContenido: { alignItems: 'center', paddingHorizontal: 4 },
  nombre: {
    color: '#fff',
    fontSize: 19,
    fontWeight: 'bold',
